...
client.on('ready', async x => {
    ...
    cron.schedule('30 20 * * *', async function () {
        newUploader(DateTime.local().toLocaleString({month: 'short', day: '2-digit'}))
    });
...
async function newUploader(day) {
        ...
        if (downloadYesOrNo) {
            ...
            if (who.medium = "anime") {
                try {
                    ...
                    const newEntry = {
                        "day": day,
                        "url": `https://aniday.s3.amazonaws.com/Months/${DateTime.local().toLocaleString({month: 'long'})}/${linkDay}/${file}`,
                    ...
                    await s3fun.uploadPicture(currentImage, DateTime.local().toLocaleString({month: 'long'}), day, file, newEntry)
                    console.log(anime)
                    console.log(file)
                ...
                const newEntry = {
                    "day": day,
                    "url": `https://aniday.s3.amazonaws.com/Months/${DateTime.local().toLocaleString({month: 'long'})}/${linkDay}/${currentImage.match(/[\\w-]+\\.(jpg|png|txt)/g)[0]}`,
                    "title": 'Unknown',
                    "description": 'Unknown',
                    "score": 'Unknown',
                    "genre": 'Unknown',
